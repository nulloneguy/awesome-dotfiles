...
local clock_format = '<span font="Inter Bold 52">%H:%M - %A</span>'
...
local locker = function(s)
	...
	awful.placement.top(wanted_poster, {
		margins = {
			top = dpi(10),
		},
	})
	...
	local check_webcam = function()
		awful.spawn.easy_async_with_shell("ls -l /dev/video* | grep /dev/video0", function(stdout)
			...
			if key == "Return" then
				...
				if input_password ~= nil then
					...
					if module_check("liblua_pam") then
						...
						rtfm:connect_signal("invoked", function()
							awful.spawn(
								[[sh -c "
	...
	awesome.connect_signal("module::lockscreen_show", function()
		if lock_again == true or lock_again == nil then
			free_keygrab()
			show_lockscreen()
		end
...
local filter_bg_image = function(wall_name, index, ap, width, height)
	...
	convert -quality 100 -brightness-contrast -20x0 ]] .. " " .. blur_filter_param .. " " .. locker_config.bg_dir .. wall_name .. [[ -gravity center -crop ]] .. ap .. [[:1 +repage -resize ]] .. width .. "x" .. height .. [[! ]] .. locker_config.tmp_wall_dir .. index .. wall_name .. [[
	"]]
	return magic
end
